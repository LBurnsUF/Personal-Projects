/*
;************************************************************************
;  Name: Logan Burns
;  Description: Common compile-time constants and globals
;************************************************************************
*/

#include "common.h"
#include <avr/pgmspace.h>

//------------------------------------LUTs----------------------------------------
const int16_t sine_lut[NPTS] PROGMEM = {
	0, 25, 50, 75, 100, 125, 150, 174,
	199, 224, 248, 272, 297, 321, 344, 368,
	391, 414, 437, 460, 482, 504, 526, 547,
	568, 589, 609, 629, 649, 668, 687, 705,
	723, 741, 758, 775, 791, 806, 822, 836,
	851, 864, 877, 890, 902, 914, 925, 935,
	945, 954, 963, 971, 979, 986, 992, 998,
	1003, 1008, 1012, 1015, 1018, 1020, 1022, 1023,
	1023, 1023, 1022, 1020, 1018, 1015, 1012, 1008,
	1003, 998, 992, 986, 979, 971, 963, 954,
	945, 935, 925, 914, 902, 890, 877, 864,
	851, 836, 822, 806, 791, 775, 758, 741,
	723, 705, 687, 668, 649, 629, 609, 589,
	568, 547, 526, 504, 482, 460, 437, 414,
	391, 368, 344, 321, 297, 272, 248, 224,
	199, 174, 150, 125, 100, 75, 50, 25,
	0, -26, -51, -76, -101, -126, -151, -175,
	-200, -225, -249, -273, -298, -322, -345, -369,
	-392, -415, -438, -461, -483, -505, -527, -548,
	-569, -590, -610, -630, -650, -669, -688, -706,
	-724, -742, -759, -776, -792, -807, -823, -837,
	-852, -865, -878, -891, -903, -915, -926, -936,
	-946, -955, -964, -972, -980, -987, -993, -999,
	-1004, -1009, -1013, -1016, -1019, -1021, -1023, -1024,
	-1024, -1024, -1023, -1021, -1019, -1016, -1013, -1009,
	-1004, -999, -993, -987, -980, -972, -964, -955,
	-946, -936, -926, -915, -903, -891, -878, -865,
	-852, -837, -823, -807, -792, -776, -759, -742,
	-724, -706, -688, -669, -650, -630, -610, -590,
	-569, -548, -527, -505, -483, -461, -438, -415,
	-392, -369, -345, -322, -298, -273, -249, -225,
	-200, -175, -151, -126, -101, -76, -51, -26
};
const int16_t triangle_lut[NPTS] PROGMEM = {
	-1024, -1008,  -992,  -976,  -960,  -944,  -928,
	-912,  -896,  -880,  -864,  -848,  -832,  -816,
	-800,  -784,  -768,  -752,  -736,  -720,  -704,
	-688,  -672,  -656,  -640,  -624,  -608,  -592,
	-576,  -560,  -544,  -528,  -512,  -496,  -480,
	-464,  -448,  -432,  -416,  -400,  -384,  -368,
	-352,  -336,  -320,  -304,  -288,  -272,  -256,
	-240,  -224,  -208,  -192,  -176,  -160,  -144,
	-128,  -112,   -96,   -80,   -64,   -48,   -32,
	-16,     0,    16,    32,    48,    64,    80,
	96,   112,   128,   144,   160,   176,   192,
	208,   224,   240,   256,   272,   288,   304,
	320,   336,   352,   368,   384,   400,   416,
	432,   448,   464,   480,   496,   512,   528,
	544,   560,   576,   592,   608,   624,   640,
	656,   672,   688,   704,   720,   736,   752,
	768,   784,   800,   816,   832,   848,   864,
	880,   896,   912,   928,   944,   960,   976,
	992,  1008,  1024,  1008,   992,   976,   960,
	944,   928,   912,   896,   880,   864,   848,
	832,   816,   800,   784,   768,   752,   736,
	720,   704,   688,   672,   656,   640,   624,
	608,   592,   576,   560,   544,   528,   512,
	496,   480,   464,   448,   432,   416,   400,
	384,   368,   352,   336,   320,   304,   288,
	272,   256,   240,   224,   208,   192,   176,
	160,   144,   128,   112,    96,    80,    64,
	48,    32,    16,     0,   -16,   -32,   -48,
	-64,   -80,   -96,  -112,  -128,  -144,  -160,
	-176,  -192,  -208,  -224,  -240,  -256,  -272,
	-288,  -304,  -320,  -336,  -352,  -368,  -384,
	-400,  -416,  -432,  -448,  -464,  -480,  -496,
	-512,  -528,  -544,  -560,  -576,  -592,  -608,
	-624,  -640,  -656,  -672,  -688,  -704,  -720,
	-736,  -752,  -768,  -784,  -800,  -816,  -832,
	-848,  -864,  -880,  -896,  -912,  -928,  -944,
	-960,  -976,  -992, -1008
};
const uint16_t note_lut[NOTE_LUT_LEN] PROGMEM = {
	28, 29, 31, 33, 35, 37, 39, 41,
	44, 46, 49, 52, 55, 58, 62, 65,
	69, 73, 78, 82, 87, 92, 98, 104,
	110, 117, 123, 131, 139, 147, 156, 165,
	175, 185, 196, 208, 220, 233, 247, 262,
	277, 294, 311, 330, 349, 370, 392, 415,
	440, 466, 494, 523, 554, 587, 622, 659,
	698, 740, 784, 831, 880, 932, 988, 1047,
	1109, 1175, 1245, 1319, 1397, 1480, 1568, 1661,
	1760, 1865, 1976, 2093, 2217, 2349, 2489, 2637,
	2794, 2960, 3136, 3322, 3520, 3729, 3951, 4186
};
const int16_t sawtooth_lut[NPTS] PROGMEM = {
	-1024, -1016, -1008, -1000,  -992,  -984,  -976,  -968,
	 -960,  -952,  -944,  -936,  -928,  -920,  -912,  -904,
	 -896,  -888,  -880,  -872,  -864,  -856,  -848,  -840,
	 -832,  -824,  -816,  -808,  -800,  -792,  -784,  -776,
	 -768,  -760,  -752,  -744,  -736,  -728,  -720,  -712,
	 -704,  -696,  -688,  -680,  -672,  -664,  -656,  -648,
	 -640,  -632,  -624,  -616,  -608,  -600,  -592,  -584,
	 -576,  -568,  -560,  -552,  -544,  -536,  -528,  -520,
	 -512,  -504,  -496,  -488,  -480,  -472,  -464,  -456,
	 -448,  -440,  -432,  -424,  -416,  -408,  -400,  -392,
	 -384,  -376,  -368,  -360,  -352,  -344,  -336,  -328,
	 -320,  -312,  -304,  -296,  -288,  -280,  -272,  -264,
	 -256,  -248,  -240,  -232,  -224,  -216,  -208,  -200,
	 -192,  -184,  -176,  -168,  -160,  -152,  -144,  -136,
	 -128,  -120,  -112,  -104,   -96,   -88,   -80,   -72,
	  -64,   -56,   -48,   -40,   -32,   -24,   -16,    -8,
	    0,     8,    16,    24,    32,    40,    48,    56,
	   64,    72,    80,    88,    96,   104,   112,   120,
	  128,   136,   144,   152,   160,   168,   176,   184,
	  192,   200,   208,   216,   224,   232,   240,   248,
	  256,   264,   272,   280,   288,   296,   304,   312,
	  320,   328,   336,   344,   352,   360,   368,   376,
	  384,   392,   400,   408,   416,   424,   432,   440,
	  448,   456,   464,   472,   480,   488,   496,   504,
	  512,   520,   528,   536,   544,   552,   560,   568,
	  576,   584,   592,   600,   608,   616,   624,   632,
	  640,   648,   656,   664,   672,   680,   688,   696,
	  704,   712,   720,   728,   736,   744,   752,   760,
	  768,   776,   784,   792,   800,   808,   816,   824,
	  832,   840,   848,   856,   864,   872,   880,   888,
	  896,   904,   912,   920,   928,   936,   944,   952,
	  960,   968,   976,   984,   992,  1000,  1008,  1016
};
const int16_t square_lut[NPTS] PROGMEM = {
	 1023,  1023,  1023,  1023,  1023,  1023,  1023,  1023,
	 1023,  1023,  1023,  1023,  1023,  1023,  1023,  1023,
	 1023,  1023,  1023,  1023,  1023,  1023,  1023,  1023,
	 1023,  1023,  1023,  1023,  1023,  1023,  1023,  1023,
	 1023,  1023,  1023,  1023,  1023,  1023,  1023,  1023,
	 1023,  1023,  1023,  1023,  1023,  1023,  1023,  1023,
	 1023,  1023,  1023,  1023,  1023,  1023,  1023,  1023,
	 1023,  1023,  1023,  1023,  1023,  1023,  1023,  1023,
	 1023,  1023,  1023,  1023,  1023,  1023,  1023,  1023,
	 1023,  1023,  1023,  1023,  1023,  1023,  1023,  1023,
	 1023,  1023,  1023,  1023,  1023,  1023,  1023,  1023,
	 1023,  1023,  1023,  1023,  1023,  1023,  1023,  1023,
	 1023,  1023,  1023,  1023,  1023,  1023,  1023,  1023,
	 1023,  1023,  1023,  1023,  1023,  1023,  1023,  1023,
	 1023,  1023,  1023,  1023,  1023,  1023,  1023,  1023,
	 1023,  1023,  1023,  1023,  1023,  1023,  1023,  1023,
	-1024, -1024, -1024, -1024, -1024, -1024, -1024, -1024,
	-1024, -1024, -1024, -1024, -1024, -1024, -1024, -1024,
	-1024, -1024, -1024, -1024, -1024, -1024, -1024, -1024,
	-1024, -1024, -1024, -1024, -1024, -1024, -1024, -1024,
	-1024, -1024, -1024, -1024, -1024, -1024, -1024, -1024,
	-1024, -1024, -1024, -1024, -1024, -1024, -1024, -1024,
	-1024, -1024, -1024, -1024, -1024, -1024, -1024, -1024,
	-1024, -1024, -1024, -1024, -1024, -1024, -1024, -1024,
	-1024, -1024, -1024, -1024, -1024, -1024, -1024, -1024,
	-1024, -1024, -1024, -1024, -1024, -1024, -1024, -1024,
	-1024, -1024, -1024, -1024, -1024, -1024, -1024, -1024,
	-1024, -1024, -1024, -1024, -1024, -1024, -1024, -1024,
	-1024, -1024, -1024, -1024, -1024, -1024, -1024, -1024,
	-1024, -1024, -1024, -1024, -1024, -1024, -1024, -1024,
	-1024, -1024, -1024, -1024, -1024, -1024, -1024, -1024,
	-1024, -1024, -1024, -1024, -1024, -1024, -1024, -1024
};
const int16_t* const waves[N_WAVES] = {
	sine_lut,
	triangle_lut,
	sawtooth_lut,
	square_lut
};

//------------------------------------Globals----------------------------------------
// PuTTY frame buffer
char scope_frame_bank[2][SCOPE_FRAME_BYTES];
volatile uint8_t scope_frame_tx = 0;
volatile uint8_t scope_frame_build = 1;
volatile uint8_t scope_seq = 0;
uint8_t scope_prev_row[SCOPE_COLS_USED];

// Synth state
Voice voices[N_VOICES];
uint16_t pfbuff[2][BLOCK_SIZE];	// DMA ping-pong buffer

volatile uint8_t cwave = SINE;	// current waveform
volatile uint8_t cbuff = 0;		// 0 or 1: which prefill buffer is active
volatile uint8_t fillflag = 0;	// set by DMA ISR when it is time to fill

uint8_t base = 3;				// key offset / octave base
uint8_t active_idx[N_VOICES];
uint8_t n_active = 0;

uint32_t freq_lut[NOTE_LUT_LEN];
int8_t note_owner[NOTE_LUT_LEN];

// circular buffer filled by USART
uint8_t keybuff[KEYLEN];

// multiply by an approximate reciprocal to emulate division
const int16_t recip_table[MAX_VOICES + 1u] = {
	0,  // unused
	RECIP_DIV(1u),
	RECIP_DIV(2u),
	RECIP_DIV(3u),
	RECIP_DIV(4u),
	RECIP_DIV(5u),
	RECIP_DIV(6u),
	RECIP_DIV(7u),
	RECIP_DIV(8u),
};

// ADSR
const uint8_t  adsr_rates[3][N_RATE_PRESETS] PROGMEM = {
	{  2,    4,    8,   16,   32,   64,  128},
	{  1,    2,    4,    8,   16,   32,   64},
	{  1,    2,    4,    8,   16,   32,   64},
};
const uint16_t adsr_ms[3][N_RATE_PRESETS] PROGMEM = {
	{819,  409,  205,  102,   51,   26,   13},
	{438,  219,  109,   55,   27,   14,    7},
	{1200, 600,  300,  150,   75,   38,   19},
};

uint8_t adsr_idx[3]  = {3, 1, 2};
uint8_t adsr_rate[3]  = {16, 2, 4};

uint32_t lfo_phase = 0;
volatile uint8_t vibrato_on = 0;

// Low-pass filter presets: 256=off, lower=more filtering
const uint16_t lpf_alpha[N_LPF_PRESETS] PROGMEM = {256, 240, 224, 192, 128};
uint8_t lpf_idx = 0;

volatile uint8_t scope_busy = 0;
volatile uint8_t scope_ready = 0;
volatile uint8_t scope_block_counter = 0;
volatile uint8_t lock_cbuff = 0;